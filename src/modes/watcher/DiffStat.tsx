...
function FileRow({
  ...
  const truncated =
    displayPath.length > pathWidth ? "…" + displayPath.slice(-(pathWidth - 1)) : displayPath;
...
export function DiffStat({ files, maxFiles, selectedIdx }: DiffStatProps) {
  ...
  const visible = maxFiles !== null && maxFiles !== undefined ? files.slice(0, maxFiles) : files;
  const hidden = files.length - visible.length;

  return (
    <box style={{ flexDirection: "column" }}>
      <box style={{ flexDirection: "row", gap: 0 }}>
        <text fg="#666" style={{ width: 4 }}>
          {"  "}
        </text>
        <text fg="#666" style={{ width: pathWidth }}>
          {"FILE"}
        </text>
        <text fg="#666" style={{ width: 7 }}>
          {"ADDED".padStart(6)}
        </text>
        <text fg="#666" style={{ width: 7 }}>
          {"REMOVED".padStart(7)}
      ...
      {hidden > 0 && <text fg="#555">{`  … and ${hidden} more  (f to browse all)`}</text>}
    </box>
  );
}
